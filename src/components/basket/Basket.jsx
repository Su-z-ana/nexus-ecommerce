...
const Basket = () => {
  ...
  const removeProductsFromFirestore = async () => {
    try {
      await Promise.all(basket.map(async (product) => {
        // Assuming you have a function to delete products from Firestore
      ...
      console.error('Error removing products from Firestore: ', error);
    }
  };
